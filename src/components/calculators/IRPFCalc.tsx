      ...
      <div className="border border-[#1A1A1A] bg-[#0F0F0F] p-6">
        ...
        {modo === "anual" ? (
          <>
            <div className="grid sm:grid-cols-2 gap-4 mb-5">
              <div>
                ...
                <p className="mt-1 text-xs text-[#606060]">Rendimiento neto anual (ingresos − gastos − cuota autónomo).</p>
              </div>
              <div>
                <label htmlFor={ccaaId} className="mb-1 block text-sm font-medium text-[#D0D0D0]">Comunidad Autónoma</label>
                <select
                  id={ccaaId}
                  value={ccaa}
                ...
                <p className="mt-1 text-xs text-[#606060]">Cada CCAA aplica su propia escala autonómica.</p>
              </div>
            </div>

            {resultado.esRegimenForal ? (
              <div className="border-2 border-[#D1FF26] bg-[#0F0F0F] p-5">
                <p className="font-semibold text-white mb-2">Régimen Foral</p>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-[#1A1A1A]">
                    {TRAMOS_CCAA_2025[ccaa].map((t, i) => {
                      ...
                      return (
                        <tr key={i}>
                          <th scope="row" className="px-3 py-2 text-left text-[#D0D0D0] font-normal">
                            {eur(t.desde)} – {t.hasta ? eur(t.hasta) : "+"}
                          </th>
                          <td className="px-3 py-2 text-right text-[#606060]">{pct(estTipo)}</td>
                          <td className="px-3 py-2 text-right font-medium text-white">{pct(t.tipo)}</td>
                          <td className="px-3 py-2 text-right font-semibold text-[#D1FF26]">{pct(estTipo + t.tipo)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor={ingTrimId} className="mb-1 block text-sm font-medium text-[#D0D0D0]">Ingresos del trimestre</label>
              <input
                id={ingTrimId}
                type="number"
                inputMode="decimal"
            ...
            <div>
              <label htmlFor={gastosTrimId} className="mb-1 block text-sm font-medium text-[#D0D0D0]">Gastos del trimestre</label>
              <input
                id={gastosTrimId}
                type="number"
                inputMode="decimal"
            ...
            <div className="sm:col-span-2">
              <label htmlFor={retId} className="mb-1 block text-sm font-medium text-[#D0D0D0]">Retenciones soportadas</label>
              <input
                id={retId}
                type="number"
                inputMode="decimal"
                ...
                className="w-full border border-[#252525] bg-[#0A0A0A] px-3 py-2 text-base text-white focus:border-[#D1FF26] focus:outline-none"
              />
              <p className="mt-1 text-xs text-[#606060]">15% en facturas a empresas (7% nuevos autónomos).</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="space-y-4">
        {modo === "anual" ? (
          resultado.esRegimenForal ? (
            ...
            <>
              <div className="border-2 border-[#D1FF26] bg-[#0F0F0F] p-5">
                <p className="tech-label text-[#D1FF26]">CUOTA IRPF TOTAL</p>
                <p className="mt-1 font-display text-3xl text-[#D1FF26]">{eur(resultado.total)}</p>
                <p className="mt-2 text-sm text-[#D0D0D0]">
                  Tipo efectivo: <strong className="text-white">{pct(resultado.tipoEfectivo)}</strong>
                </p>
              </div>
              <div className="border border-[#1A1A1A] bg-[#0F0F0F] p-5 text-sm space-y-2">
                <div className="flex justify-between">
                  <span className="text-[#606060]">Estatal</span>
                  <span className="font-medium text-white">{eur(resultado.estatal)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#606060]">{CCAA_NAMES[ccaa]}</span>
                  <span className="font-medium text-white">{eur(resultado.autonomico)}</span>
                </div>
                <div className="border-t border-[#1A1A1A] pt-2 flex justify-between">
                  <span className="font-semibold text-white">Total</span>
                  <span className="font-bold text-white">{eur(resultado.total)}</span>
                </div>
              </div>
              <div className="border border-[#1A1A1A] bg-[#0F0F0F] p-5 text-xs text-[#D0D0D0]">
          ...
          <>
            <div className="border-2 border-[#D1FF26] bg-[#0F0F0F] p-5">
              <p className="tech-label text-[#D1FF26]">A PAGAR (MODELO 130)</p>
              <p className="mt-1 font-display text-3xl text-[#D1FF26]">{eur(pago130)}</p>
              <p className="mt-2 text-sm text-[#D0D0D0]">20% sobre rendimiento neto – retenciones</p>
            </div>
            <div className="border border-[#1A1A1A] bg-[#0F0F0F] p-5 text-sm text-[#D0D0D0]">
              Solo presentas modelo 130 si <strong className="text-white">menos del 70%</strong> de tus ingresos
              llevan retención.
            </div>
          </>
        )}

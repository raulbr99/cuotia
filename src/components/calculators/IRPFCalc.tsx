      ...
      <div className="rounded-xl border border-[#1A1A1A] bg-[#0F0F0F] p-6 shadow-sm">
          ...
          {(["anual", "trimestral"] as Modo[]).map((m) => (
            <button
              ...
              className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
                modo === m ? "bg-[#D1FF26] text-[#0A0A0A]" : "bg-[#1A1A1A] text-[#A0A0A0] hover:bg-[#252525]"
              }`}
            >
        ...
        {modo === "anual" ? (
          <>
            <div className="grid sm:grid-cols-2 gap-4 mb-5">
              <div>
                ...
                <p className="text-xs text-[#606060] mt-1">Rendimiento neto anual (ingresos − gastos − cuota autónomo).</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-[#A0A0A0] mb-1">Comunidad Autónoma</label>
                <select
                  value={ccaa}
                  onChange={(e) => setCcaa(e.target.value as CCAA)}
                  className="w-full rounded-lg border border-[#252525] px-3 py-2 text-base focus:outline-none focus:ring-2 focus:ring-[#D1FF26] bg-[#0F0F0F]"
                >
                  {ccaaOptions.map((c) => (
                    <option key={c} value={c}>{CCAA_NAMES[c]}</option>
                  ))}
                </select>
                <p className="text-xs text-[#606060] mt-1">Cada CCAA aplica su propia escala autonómica.</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[#A0A0A0] mb-1">Ingresos del trimestre</label>
              <input
                type="number"
                value={ingresosTrim}
            ...
            <div>
              <label className="block text-sm font-medium text-[#A0A0A0] mb-1">Gastos del trimestre</label>
              <input
                type="number"
                value={gastosTrim}
            ...
            <div className="col-span-2">
              <label className="block text-sm font-medium text-[#A0A0A0] mb-1">Retenciones soportadas</label>
              <input
                type="number"
                value={retenciones}
                ...
                className="w-full rounded-lg border border-[#252525] px-3 py-2 text-base focus:outline-none focus:ring-2 focus:ring-[#D1FF26]"
              />
              <p className="text-xs text-[#606060] mt-1">15% en facturas a empresas (7% nuevos autónomos).</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="space-y-4">
        {modo === "anual" ? (
          resultado.esRegimenForal ? (
            ...
            <>
              <div className="rounded-xl border-2 border-[#D1FF26] bg-[#0F0F0F] p-5">
                <p className="text-xs uppercase tracking-wider text-[#D1FF26] font-medium">Cuota IRPF total</p>
                <p className="text-3xl font-bold text-[#D1FF26] mt-1">{eur(resultado.total)}</p>
              ...
              <div className="rounded-xl border border-[#1A1A1A] bg-[#0F0F0F] p-5 text-xs text-[#A0A0A0]">
          ...
          <>
            <div className="rounded-xl border-2 border-[#D1FF26] bg-[#0F0F0F] p-5">
              <p className="text-xs uppercase tracking-wider text-[#D1FF26] font-medium">A pagar (modelo 130)</p>
              <p className="text-3xl font-bold text-[#D1FF26] mt-1">{eur(pago130)}</p>
              <p className="text-sm text-[#D1FF26] mt-2">20% sobre rendimiento neto – retenciones</p>
            </div>
            <div className="rounded-xl border border-[#1A1A1A] bg-[#0F0F0F] p-5 text-sm text-[#A0A0A0]">

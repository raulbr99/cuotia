        ...
        {(["agregar", "quitar", "303"] as Modo[]).map((m) => (
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-medium ${
              modo === m ? "bg-emerald-600 text-white" : "bg-gray-100 hover:bg-gray-200"
            }`}
          >
      ...
      {modo !== "303" ? (
        <div className="grid gap-6 md:grid-cols-[1fr_320px]">
                ...
                {(Object.keys(TIPOS_IVA) as TipoIVA[]).map((t) => (
                  <button
                    ...
                    className={`rounded-lg border px-3 py-2 text-sm ${
                      tipo === t ? "border-emerald-500 bg-emerald-50" : "border-gray-200"
                    }`}
                  >
                    <p className="font-medium capitalize">{t}</p>
                    <p className="text-xs text-gray-500">{(TIPOS_IVA[t] * 100).toFixed(0)}%</p>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="rounded-xl border-2 border-emerald-200 bg-emerald-50 p-5 space-y-3">
            <div>
              <p className="text-xs uppercase tracking-wider text-emerald-700 font-medium">Total</p>
              <p className="text-3xl font-bold text-emerald-900">
                {eur(modo === "agregar" ? agregar.total : quitar.total)}
              </p>
            </div>
            <div className="text-sm space-y-1 pt-3 border-t border-emerald-200">
        ...
        <div className="grid gap-6 md:grid-cols-[1fr_320px]">
          ...
          <div className="rounded-xl border-2 border-emerald-200 bg-emerald-50 p-5">
            ...
            <p className="text-3xl font-bold text-emerald-900 mt-1">{eur(m303.resultado)}</p>
